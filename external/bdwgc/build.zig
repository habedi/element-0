...
const zig_min_required_version = "0.14.1";
...
pub fn build(b: *std.Build) void {
    ...
    if (enable_threads) {
        ...
        if (t.os.tag != .windows) { // assume `pthreads`
            ...
            flags.append(b.allocator, "-D GC_BUILTIN_ATOMIC") catch unreachable;
            if (enable_thread_local_alloc and (enable_parallel_mark or !build_shared_libs)) {
    ...
    if (build_cord) {
        cord = b.addLibrary(.{ .linkage = if (build_shared_libs) .dynamic else .static, .name = "cord", .root_module = b.createModule(.{
            .target = target,
            .optimize = optimize,
        }) });
    ...
    if (build_cord) {
        addTestExt(b, gc, cord, test_step, flags, "cordtest", "cord/tests/cordtest.c");
    ...
    if (enable_cplusplus) {
        ...
        if (enable_throw_bad_alloc_library) {
            addTestExt(b, gc, gctba, test_step, flags, "treetest", "tests/tree.cc");
        }
    }
    if (enable_disclaim) {
...
fn addTestExt(b: *std.Build, gc: *std.Build.Step.Compile, lib2: ?*std.Build.Step.Compile, test_step: *std.Build.Step, flags: std.ArrayListUnmanaged([]const u8), testname: []const u8, filename: []const u8) void {
    const test_exe = b.addExecutable(.{ .name = testname, .root_module = b.createModule(.{ .optimize = gc.root_module.optimize.?, .target = gc.root_module.resolved_target.? }) });
    test_exe.addCSourceFile(.{ .file = b.path(filename), .flags = flags.items });
